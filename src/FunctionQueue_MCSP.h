...
private:
    ...
    public:
        ...
        inline void free() noexcept { fp_offset.store(0, std::memory_order_release); }

        [[nodiscard]] inline bool isFree() const noexcept { return fp_offset.load(std::memory_order_acquire) == 0; }
...
private:
    ...
    static inline void *align(void *&ptr, size_t &space) noexcept {
        ...
        if ((sizeof(T) + diff) > space) return nullptr;
        else {
            space -= diff;
