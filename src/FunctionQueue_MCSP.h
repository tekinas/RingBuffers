...
template <typename T, bool isWriteProtected, bool isIndexed = false,
          bool destroyNonInvoked = true>
class FunctionQueue_MCSP {};

template <typename R, typename... Args, bool isWriteProtected, bool isIndexed,
          bool destroyNonInvoked>
class FunctionQueue_MCSP<R(Args...), isWriteProtected, isIndexed,
                         destroyNonInvoked> {
private:
  ...
  public:
    inline auto getReadData() noexcept {
      return std::tuple{
          std::bit_cast<InvokeAndDestroy>(
              fp_offset.load(std::memory_order_relaxed) + fp_base),
...
public:
  FunctionQueue_MCSP(void *memory, std::size_t size) noexcept
...
private:
  ...
  template <typename T>
  static inline void *align(void *&ptr, size_t &space) noexcept {
        ...
        if (followOffset == m_SentinelFollow) {
          if (m_SentinelRead.load(std::memory_order_relaxed) == NO_SENTINEL) {
            ...
            m_SentinelFollow = NO_SENTINEL;
                       ...
                       align<FunctionContext>(m_Buffer + followOffset);
                   functionCxt->isFree()) {
