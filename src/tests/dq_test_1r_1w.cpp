...
void test_buffer_queue(BufferQueue &buffer_queue, RNG &rng,
...
int main(int argc, char **argv) {
  ...
  size_t const buffer_size = [&] {
    return (argc >= 2) ? atof(argv[1]) : 10000 / 1024.0 / 1024.0;
  }() * 1024 * 1024;
  ...
  size_t const buffers = [&] {
    return (argc >= 4) ? atol(argv[3]) : 1000000;
  }();
    ...
    while (func) {
      ...
      buffer_queue.allocate_and_release(
          ...
          [&, min_buffer_size](auto buffer, auto avl_size) mutable {
            ...
            std::span const data{reinterpret_cast<char *>(buffer), fill_bytes};
            rng.fillRand<char>(std::numeric_limits<char>::min(),
                               std::numeric_limits<char>::max(), data.begin(),
                               data.end());
            --func;
            return fill_bytes;
          });
