...
int main(int argc, char **argv) {
  ...
  println("std::dequeue<folly::Function> storage :",
          computeDequeueStorage / ONE_MB, " Mb");
  println("std::dequeue<std::function> storage :",
          computeStdDequeueStorage / ONE_MB, " Mb");
...
void test(std::deque<Function<ComputeFunctionSig>> &computeDequeue) noexcept {
  size_t num = 0;
  {
    ...
    for (auto end = computeDequeue.end(), begin = computeDequeue.begin();
         begin != end; ++begin) {
      num = (*begin)(num);
    }
  }
...
void test(
    std::deque<std::function<ComputeFunctionSig>> &computeStdDequeue) noexcept {
  size_t num = 0;
  {
    Timer timer{"std::deque of std functions"};
    for (auto end = computeStdDequeue.end(), begin = computeStdDequeue.begin();
         begin != end; ++begin) {
      num = (*begin)(num);
    }
  }
