...
using ComputeFunctionQueue =
    FunctionQueue_MCSP<ComputeFunctionSig, true, false, false>;
...
int main(int argc, char **argv) {
  ...
  size_t const rawQueueMemSize = [&] {
    return (argc >= 2) ? atof(argv[1]) : 1024 + 150;
  }() * 1024 * 1024;
  ...
  size_t const numWriterThreads = [&] {
    return (argc >= 4) ? atol(argv[3]) : std::thread::hardware_concurrency();
  }();
...

    {
      ...
      for (auto t = numReaderThreads; t--;) {
        reader_threads.emplace_back(
            [=, &rawComputeQueue, &result_vector, &res_vec_mut] {
              ...
              result_vector.insert(result_vector.end(), res_vec.begin(),
                                   res_vec.end());
            });
      }
    }
