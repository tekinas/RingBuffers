...
int main(int argc, char **argv) {
  if (argc == 1) {
    println("usage : ./fq_test_call_only <buffer_size> <seed>");
  }

  size_t const rawQueueMemSize = [&] {
    return (argc >= 2) ? atof(argv[1]) : 500.0;
  }() * 1024 * 1024;
  ...
  size_t const compute_functors = [&] {
    ...
    while (addFunction) {
      callbackGenerator.addCallback([&]<typename T>(T &&t) {
        addFunction = rawComputeQueue.push_back(std::forward<T>(t));
      });
    }
  ...
  callbackGenerator.setSeed(seed);
  {
    ...
    for (auto count = compute_functors; count--;) {
      callbackGenerator.addCallback([&]<typename T>(T &&t) {
        computeVector.emplace_back(std::forward<T>(t));
      });
    }
  }
  ...
  callbackGenerator.setSeed(seed);
  {
    ...
    for (auto count = compute_functors; count--;) {
      callbackGenerator.addCallback([&]<typename T>(T &&t) {
        computeStdVector.emplace_back(std::forward<T>(t));
      });
    }
  }
  ...
  println("std::vector<folly::Function> storage :",
          computeVectorStorage / ONE_MB, " Mb");
  println("std::vector<std::function> storage :",
          computeStdVectorStorage / ONE_MB, " Mb");
...
void test(
    std::vector<Function<ComputeFunctionSig>> &vectorComputeQueue) noexcept {
  size_t num = 0;
  {
...
void test(std::vector<std::function<ComputeFunctionSig>>
              &vectorStdComputeQueue) noexcept {
  size_t num = 0;
  {
