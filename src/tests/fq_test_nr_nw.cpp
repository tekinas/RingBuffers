...
public:
  explicit ComputeCxt(size_t seed, size_t num_functions,
                      std::string_view timer_str)
      : num_functions{num_functions}, callbackGenerator{seed}, timer{
                                                                   timer_str} {}

  static void addComputeTask(std::unique_ptr<ComputeCxt> computeCxt,
                             ComputeFunctionQueue *functionQueue) noexcept {
    auto const cxtPtr = computeCxt.get();
    cxtPtr->callbackGenerator.addCallback(
        [computeCxt{std::move(computeCxt)},
         functionQueue]<typename T>(T &&t) mutable {
          auto compute = [computeCxt{std::move(computeCxt)},
                          t{std::forward<T>(t)}, functionQueue]() mutable {
...
int main(int argc, char **argv) {
  ...
  for (auto t = compute_chains; t--;)
    ComputeCxt::addComputeTask(
        std::make_unique<ComputeCxt>(seed, functions,
                                     "compute chain " + std::to_string(t + 1)),
        &rawComputeQueue);
