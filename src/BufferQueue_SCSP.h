...
template <bool isReadProtected, bool isWriteProtected,
...
public:
  ...
  public:
    ...
    ~Buffer() noexcept {
      if (buffer_queue) {
        uint32_t const nextOffset =
            getDataContext()->getNextAddr() - buffer_queue->m_Buffer;
        buffer_queue->m_Remaining.fetch_sub(1, std::memory_order_relaxed);
        if constexpr (isReadProtected) {
          buffer_queue->m_OutPutOffset.store(nextOffset,
  ...
  private:
    ...
    explicit Buffer(BufferQueue_SCSP const *buffer_queue) noexcept
        : buffer_queue{buffer_queue} {}

    auto getDataContext() const noexcept {
      return std::bit_cast<DataContext *>(
          buffer_queue->m_Buffer +
          buffer_queue->m_OutPutOffset.load(std::memory_order_relaxed));
    }
...
public:
  ...
  inline decltype(auto) consume(F &&functor) const noexcept {
    ...
    auto const data_cxt =
        align<DataContext>(m_Buffer + (found_sentinel ? 0 : output_offset));
    auto const [buffer_data, buffer_size] = data_cxt->getBuffer();

    auto decr_rem_incr_output_offset =
        [&, nextOffset{data_cxt->getNextAddr() - m_Buffer}] {
  ...
  inline auto consume() const noexcept {
    ...
    m_OutPutOffset.store(align<std::byte, alignof(DataContext)>(
                             m_Buffer + (found_sentinel ? 0 : output_offset)) -
                             m_Buffer,
                         std::memory_order_relaxed);
