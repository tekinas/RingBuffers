...
public:
  ...
  public:
    ...
    inline ObjectType &operator*() const noexcept {
      return object_queue->m_Array[object_queue->m_OutputIndex.load(
          std::memory_order_relaxed)];
    }

    inline ObjectType *get() const noexcept {
      return object_queue->m_Array +
             object_queue->m_OutputIndex.load(std::memory_order_relaxed);
    }
    ...
    inline ~Ptr() noexcept {
      if (object_queue) {
        ...
        auto const nextIndex = output_index == object_queue->m_LastElementIndex
                                   ? 0
                                   : (output_index + 1);
        if constexpr (isReadProtected) {
          object_queue->m_OutputIndex.store(nextIndex,
                                            std::memory_order_relaxed);
  ...
  private:
    ...
    explicit Ptr(ObjectQueue_SCSP const *object_queue) noexcept
...
public:
  inline ObjectQueue_SCSP(ObjectType *buffer, uint32_t count)
      : m_Array{buffer}, m_LastElementIndex{count - 1} {
    if constexpr (isReadProtected)
  ...
  inline bool empty() const noexcept {
    return m_InputIndex.load(std::memory_order_acquire) ==
           m_OutputIndex.load(std::memory_order_relaxed);
  }

  template <typename F> decltype(auto) consume(F &&functor) const noexcept {
    auto const output_index = m_OutputIndex.load(std::memory_order_relaxed);
    ...
    auto cleanup = [&,
      ...
      auto const nextIndex =
          output_index == m_LastElementIndex ? 0 : (output_index + 1);
      if constexpr (isReadProtected) {
    ...
    using ReturnType =
        decltype(std::forward<F>(functor)(std::declval<ObjectType &>()));
  ...
  template <typename F> uint32_t consume_all(F &&functor) const noexcept {
    auto const output_index = m_OutputIndex.load(std::memory_order_relaxed);
    auto const input_index = m_InputIndex.load(std::memory_order_acquire);
  ...
  template <typename T>
  requires std::same_as<std::decay_t<T>, ObjectType> bool
  push_back(T &&obj) noexcept {
    if constexpr (isWriteProtected) {
    ...
    auto const next_input_index =
        input_index == m_LastElementIndex ? 0 : (input_index + 1);

    if (next_input_index == output_index)
      return false;
  ...
  uint32_t emplace_back_n(Functor &&functor) noexcept {
    ...
    auto const count_avl = (input_index < output_index)
                               ...
                               : (m_LastElementIndex - input_index +
                                  static_cast<bool>(output_index));

    if (!count_avl)
      return 0;

    auto const count_emplaced =
        std::forward<Functor>(functor)(m_Array + input_index, count_avl);
    auto const input_end = input_index + count_emplaced;
    auto const next_input_index =
        (input_end == (m_LastElementIndex + 1)) ? 0 : input_end;
    ...
    if constexpr (!std::is_trivially_destructible_v<ObjectType>) {
      ...
      if (output_index > input_index) {
        std::destroy_n(m_Array + output_index,
                       m_LastElementIndex - output_index + 1);
...
private:
  ...
  [[no_unique_address]] std::conditional_t<isWriteProtected, std::atomic_flag,
                                           Null>
      m_WriteFlag;
  [[no_unique_address]] mutable std::conditional_t<isReadProtected,
                                                   std::atomic_flag, Null>
      m_ReadFlag;
